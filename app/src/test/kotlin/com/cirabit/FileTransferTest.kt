...
class FileTransferTest {
    ...
    fun `replaceFilePathInContent should correctly format content markers for different file types`() {
        ...
        val imageMessage = CirabitMessage(
            ...
            content = "/data/user/0/com.cirabit.android/files/images/photo.jpg",
            type = CirabitMessageType.Image,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val audioMessage = CirabitMessage(
            id = "test2",
            sender = "bob",
            senderPeerID = "87654321",
            content = "/data/user/0/com.cirabit.android/files/audio/voice.amr",
            type = CirabitMessageType.Audio,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )

        val fileMessage = CirabitMessage(
            id = "test3",
            sender = "charlie",
            senderPeerID = "11223344",
            content = "/data/user/0/com.cirabit.android/files/documents/document.pdf",
            type = CirabitMessageType.File,
            timestamp = Date(System.currentTimeMillis()),
            isPrivate = false
        )
        ...
        result = result.replace(
            "/data/user/0/com.cirabit.android/files/images/photo.jpg",
            "[image] photo.jpg"
        )
