...
import com.cirabit.android.model.PrivateMessagePacket
import com.cirabit.android.protocol.CirabitPacket
import com.cirabit.android.services.SeenMessageStore
import com.cirabit.android.ui.ChatState
import com.cirabit.android.ui.MeshDelegateHandler
import com.cirabit.android.ui.PrivateChatManager
...
class NostrDirectMessageHandler(
    ...
    private val dataManager: com.cirabit.android.ui.DataManager
) {
    ...
    fun onGiftWrap(giftWrap: NostrEvent, geohash: String, identity: NostrIdentity) {
        scope.launch(Dispatchers.Default) {
            try {
                ...
                if (!content.startsWith("cirabit1:")) return@launch

                val base64Content = content.removePrefix("cirabit1:")
                val packetData = base64URLDecode(base64Content) ?: return@launch
                val packet = CirabitPacket.fromBinaryData(packetData) ?: return@launch

                if (packet.type != com.cirabit.android.protocol.MessageType.NOISE_ENCRYPTED.value) return@launch
                ...
                com.cirabit.android.nostr.GeohashAliasRegistry.put(convKey, senderPubkey)
    ...
    private suspend fun processNoisePayload(
        ...
        when (payload.type) {
            ...
            NoisePayloadType.FILE_TRANSFER -> {
                ...
                val file = CirabitFilePacket.decode(payload.data)
                if (file != null) {
                    val uniqueMsgId = java.util.UUID.randomUUID().toString().uppercase()
                    val savedPath = com.cirabit.android.features.file.FileUtils.saveIncomingFile(application, file)
                    val message = CirabitMessage(
                        id = uniqueMsgId,
                        sender = senderNickname,
                        content = savedPath,
