...
import com.cirabit.android.crypto.EncryptionService
...
class BluetoothMeshService(private val context: Context) {
    private val debugManager by lazy { try { com.cirabit.android.ui.debug.DebugSettingsManager.getInstance() } catch (e: Exception) { null } }
    
    companion object {
        private const val TAG = "BluetoothMeshService"
        private val MAX_TTL: UByte = com.cirabit.android.util.AppConstants.MESSAGE_TTL_HOPS
    }
    ...
    private val serviceNotificationManager = com.cirabit.android.ui.NotificationManager(
        context.applicationContext,
        androidx.core.app.NotificationManagerCompat.from(context.applicationContext),
        com.cirabit.android.util.NotificationIntervalManager()
    )
    ...
    init {
        ...
        gossipSyncManager = GossipSyncManager(
            ...
            configProvider = object : GossipSyncManager.ConfigProvider {
                override fun seenCapacity(): Int = try {
                    com.cirabit.android.ui.debug.DebugPreferenceManager.getSeenPacketCapacity(500)
                } catch (_: Exception) { 500 }

                override fun gcsMaxBytes(): Int = try {
                    com.cirabit.android.ui.debug.DebugPreferenceManager.getGcsMaxFilterBytes(400)
                } catch (_: Exception) { 400 }

                override fun gcsTargetFpr(): Double = try {
                    com.cirabit.android.ui.debug.DebugPreferenceManager.getGcsFprPercent(1.0) / 100.0
        ...
        gossipSyncManager.delegate = object : GossipSyncManager.Delegate {
            override fun sendPacket(packet: CirabitPacket) {
                connectionManager.broadcastPacket(RoutedPacket(packet))
            }
            override fun sendPacketToPeer(peerID: String, packet: CirabitPacket) {
                connectionManager.sendPacketToPeer(peerID, packet)
            }
            override fun signPacketForBroadcast(packet: CirabitPacket): CirabitPacket {
    ...
    private fun setupDelegates() {
        ...
        peerManager.delegate = object : PeerManagerDelegate {
            override fun onPeerListUpdated(peerIDs: List<String>) {
                ...
                try { com.cirabit.android.services.AppStateStore.setPeers(peerIDs) } catch (_: Exception) { }
            ...
            override fun onPeerRemoved(peerID: String) {
                ...
                try { com.cirabit.android.services.meshgraph.MeshGraphService.getInstance().removePeer(peerID) } catch (_: Exception) { }
        ...
        securityManager.delegate = object : SecurityManagerDelegate {
            ...
            override fun sendHandshakeResponse(peerID: String, response: ByteArray) {
                ...
                val responsePacket = CirabitPacket(
                    version = 1u,
        ...
        messageHandler.delegate = object : MessageHandlerDelegate {
            ...
            override fun verifySignature(packet: CirabitPacket, peerID: String): Boolean {
            ...
            override fun updatePeerIDBinding(newPeerID: String, nickname: String,
                ...
                try {
                    com.cirabit.android.favorites.FavoritesPersistenceService.shared.findNostrPubkey(publicKey)?.let { npub ->
                        com.cirabit.android.favorites.FavoritesPersistenceService.shared.updateNostrPublicKeyForPeerID(newPeerID, npub)
                    }
                } catch (_: Exception) { }
            ...
            // Callbacks
            override fun onMessageReceived(message: CirabitMessage) {
                // Always reflect into process-wide store so UI can hydrate after recreation
                try {
                    when {
                        message.isPrivate -> {
                            val peer = message.senderPeerID ?: ""
                            if (peer.isNotEmpty()) com.cirabit.android.services.AppStateStore.addPrivateMessage(peer, message)
                        }
                        message.channel != null -> {
                            com.cirabit.android.services.AppStateStore.addChannelMessage(message.channel!!, message)
                        }
                        else -> {
                            com.cirabit.android.services.AppStateStore.addPublicMessage(message)
                        }
                    }
                } catch (_: Exception) { }
                ...
                if (delegate == null && message.isPrivate) {
                    try {
                        ...
                        if (senderPeerID != null) {
                            ...
                            val preview = com.cirabit.android.ui.NotificationTextUtils.buildPrivateMessagePreview(message)
        ...
        packetProcessor.delegate = object : PacketProcessorDelegate {
            override fun validatePacketSecurity(packet: CirabitPacket, peerID: String): Boolean {
            ...
            override fun handleAnnounce(routed: RoutedPacket) {
                serviceScope.launch {
                    ...
                    if (deviceAddress != null && pid != null) {
                        ...
                        val isDirect = routed.packet.ttl == com.cirabit.android.util.AppConstants.MESSAGE_TTL_HOPS
            ...
            override fun handleFragment(packet: CirabitPacket): CirabitPacket? {
        ...
        override fun onPacketReceived(packet: CirabitPacket, peerID: String, device: android.bluetooth.BluetoothDevice?) {
            // Log incoming for debug graphs (do not double-count anywhere else)
            try {
                com.cirabit.android.ui.debug.DebugSettingsManager.getInstance().logIncoming(
                    packet = packet,
                    fromPeerID = peerID,
                    fromNickname = null,
            ...
            override fun onDeviceConnected(device: android.bluetooth.BluetoothDevice) {
                ...
                try {
                    ...
                    com.cirabit.android.ui.debug.DebugSettingsManager.getInstance()
            ...
            serviceScope.launch {
                ...
                if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                    try {
                        ...
                        val noisePayload = com.cirabit.android.model.NoisePayload(
                            type = com.cirabit.android.model.NoisePayloadType.FILE_TRANSFER,
                            data = filePayload
                        )
        ...
        serviceScope.launch {
            ...
            if (encryptionService.hasEstablishedSession(recipientPeerID)) {
                try {
                    ...
                    val privateMessage = com.cirabit.android.model.PrivateMessagePacket(
                    ...
                    val messagePayload = com.cirabit.android.model.NoisePayload(
                        type = com.cirabit.android.model.NoisePayloadType.PRIVATE_MESSAGE,
                        data = tlvData
                    )
    ...
    fun sendReadReceipt(messageID: String, recipientPeerID: String, readerNickname: String) {
        serviceScope.launch {
            ...
            val geo = runCatching { com.cirabit.android.services.MessageRouter.tryGetInstance() }.getOrNull()
            val isGeoAlias = try {
                val map = com.cirabit.android.nostr.GeohashAliasRegistry.snapshot()
                map.containsKey(recipientPeerID)
            } catch (_: Exception) { false }
            if (isGeoAlias && geo != null) {
                geo.sendReadReceipt(com.cirabit.android.model.ReadReceipt(messageID), recipientPeerID)
                return@launch
            }

            try {
                // Avoid duplicate read receipts: check persistent store first
                val seenStore = try { com.cirabit.android.services.SeenMessageStore.getInstance(context.applicationContext) } catch (_: Exception) { null }
                ...
                val readReceiptPayload = com.cirabit.android.model.NoisePayload(
                    type = com.cirabit.android.model.NoisePayloadType.READ_RECEIPT,
                ...
                val packet = CirabitPacket(
                    ...
                    payload = encrypted,
                    signature = null,
                    ttl = com.cirabit.android.util.AppConstants.MESSAGE_TTL_HOPS // Same TTL as iOS messageTTL
                )
    ...
    fun sendBroadcastAnnounce() {
        ...
        serviceScope.launch {
            val nickname = try { com.cirabit.android.services.NicknameProvider.getNickname(context, myPeerID) } catch (_: Exception) { myPeerID }
     ...
     */
    fun getSessionState(peerID: String): com.cirabit.android.noise.NoiseSession.NoiseSessionState {
     ...
     */
    private fun signPacketBeforeBroadcast(packet: CirabitPacket): CirabitPacket {
        return try {
            ...
            val withRoute = try {
                ...
                if (rec != null && !rec.contentEquals(SpecialRecipients.BROADCAST)) {
                    ...
                    val path = com.cirabit.android.services.meshgraph.RoutePlanner.shortestPath(myPeerID, dest)
