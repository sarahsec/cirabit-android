...
data class CirabitPacket(
    ...
    fun toBinaryDataForSigning(): ByteArray? {
        ...
        val unsignedPacket = CirabitPacket(
            version = version,
            type = type,
            senderID = senderID,
            ...
            route = route,
            ttl = com.cirabit.android.util.AppConstants.SYNC_TTL_HOPS // Use fixed TTL=0 for signing to ensure relay compatibility
        )
        return BinaryProtocol.encode(unsignedPacket)
    }

    companion object {
        fun fromBinaryData(data: ByteArray): CirabitPacket? {
...
object BinaryProtocol {
    ...
    fun decode(data: ByteArray): CirabitPacket? {
     ...
     */
    private fun decodeCore(raw: ByteArray): CirabitPacket? {
        try {
